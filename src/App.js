  ...
  const processForecastData = (forecastData) => {
    ...
    return Object.values(dailyData)
      ...
      .map((day) => ({
        ...
        humidity: Math.round(
          day.humidities.reduce((sum, humidity) => sum + humidity, 0) /
            day.humidities.length
        ),
        windSpeed: (
          day.windSpeeds.reduce((sum, speed) => sum + speed, 0) /
    ...
    <AppContainer>
      ...
      {weatherData && weatherData[unit] && (
        <>
          ...
          {coordinates && <Map coordinates={coordinates} location={location} />}

          {weatherData[unit].forecastData.length > 0 && (
            <ForecastContainer>
              <h2 style={{ color: '#ffffff' }}>5-Day Forecast</h2>
